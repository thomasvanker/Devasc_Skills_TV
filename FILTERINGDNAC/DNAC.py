...
resp_devices = requests.get(req_url, headers=headers, verify=False)
...
for device in resp_devices_json['response']:
    if device['type'] is not None:
        ...
        print('Reachability: ' + 'Reachable' if device.get('reachabilityStatus') == 'REACHABLE' else 'Unreachable')
